function [ orbitFilename, Nrow ] = mkOrbitFile(tStart,tEnd,Norb,omni_path)
%mkOrbitFile Create orbit file from OMNIWeb file
...
%   orbitFilename: Created orbit filename
%   length       : Row number
%
...
%
    ...
    % Array declaration
    [status,result]=unix(['wc -l ',omni_path,'/temp_orbit.dat']);
    Nrow=str2num(result(1:findstr(result,' ')));
    A=zeros(Nrow,12);
    t=(1:Nrow);
    % Read the temporary OMNIWeb file
    fid=fopen([omni_path,'/temp_orbit.dat'], 'r');   
    % Reading data
    i=1;   
	strLine=fgetl(fid);
	while (feof(fid)==0)
    ...
    orbitFilename=['orbit',num2str(Norb),'-',strTstart,'_',strTend,'.dat'];   
    fid=fopen([omni_path,orbitFilename], 'w');   
    % Reading data
    for i=tiStart:tiEnd
    ...
    % Purge garbage
    [status,result]=unix(['rm ',omni_path,'temp_orbit.dat']);
end
